---
title: "Associate Data Scientist"
collection: certifications
type: "Professional Certification"
issuer: "DataCamp"
date: 2024-11-01
image: "certifications/datacamp-associate-data-scientist.png"
link: "https://www.datacamp.com/completed/statement-of-accomplishment/track/a63516e3344d6f4fdee72dca092672763295f043"
excerpt: "Track covering Python, SQL, statistics, probability, data manipulation & visualization and predictive modeling."
---

Earned the **Associate Data Scientist** certification from DataCamp.

## Topics Covered

- **Python**: Programming fundamentals and data science libraries
- **SQL**: Database querying and data extraction
- **Statistics & Probability**: Foundational concepts for data analysis
- **Data Manipulation & Visualization**: Cleaning data and creating visualizations
- **Predictive Modeling**: Building and evaluating ML models

## Certificate

[View Certificate](https://www.datacamp.com/completed/statement-of-accomplishment/track/a63516e3344d6f4fdee72dca092672763295f043)
